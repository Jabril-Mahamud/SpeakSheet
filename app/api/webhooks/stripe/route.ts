    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        try {
          ...
          if (!item) {
            console.error("No items on subscription:", subscription.id);
            return NextResponse.json(
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            current_period_start: (() => {
              const updItem = (subscriptionData.items as any).data[0];
              return updItem
                ? new Date(updItem.current_period_start * 1000).toISOString()
                : undefined;
            })(),
            current_period_end: (() => {
              const updItem = (subscriptionData.items as any).data[0];
              return updItem
                ? new Date(updItem.current_period_end * 1000).toISOString()
                : undefined;
            })(),            
